...
CREATE TABLE employees (
    employee_id INT PRIMARY KEY, first_name VARCHAR(50), last_name VARCHAR(50), email VARCHAR(50), hire_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, gender VARCHAR(1), -- "M"/"F" (male/female)
    salary INT, coffeeshop_id INT
);

-- Create shops table
CREATE TABLE shops (
    coffeeshop_id INT PRIMARY KEY, coffeeshop_name VARCHAR(50), city_id INT
);

-- Add foreign key to the employees table
ALTER TABLE employees
ADD FOREIGN KEY (coffeeshop_id) REFERENCES shops (coffeeshop_id) ON DELETE SET NULL;

-- Create locations table
CREATE TABLE locations (
    city_id INT PRIMARY KEY, city VARCHAR(50), country VARCHAR(50)
);

-- Add foreign key to shops table
ALTER TABLE shops
ADD FOREIGN KEY (city_id) REFERENCES locations (city_id) ON DELETE SET NULL;
...
-- Active: 1712521498825@@127.0.0.1@3306@coffee#Query
...
INSERT INTO
    employees
VALUES (
        220691, 'Bernelle', 'Lacoste', 'blacoste3u@studiopress.com', '2015/01/01', 'F', 41560, 3
    ),
    (
        498390, 'Vikky', 'Caswell', NULL, '2019/02/22', 'F', 22659, 1
    ),
    (
        189665, 'Maddy', 'Janouch', 'mjanouch3w@icio.us', '2015/12/05', 'M', 50384, 4
    ),
    (
        221890, 'Bliss', 'Whitecross', NULL, '2018/05/18', 'F', 24871, 4
    ),
    (
        237255, 'Giorgi', 'Mignot', 'gmignot3y@lulu.com', '2022/07/11', 'M', 51983, 4
    ),
    (
        92481, 'Sherrie', 'Titman', NULL, '2014/04/06', 'F', 18894, 5
    ),
    (
        468001, 'Evvie', 'Cotterell', 'ecotterell40@google.co.uk', '2018/06/20', 'F', 16390, 2
    ),
    (
        814214, 'Stephine', 'Cicccitti', 'scicccitti41@baidu.com', '2018/07/13', 'F', 35309, 3
    ),
    (
        78629, 'Daniele', 'Pickthall', 'dpickthall42@ox.ac.uk', '2013/12/30', 'F', 60276, 5
    ),
    (
        201598, 'Leigha', 'Cullin', NULL, '2017/05/10', 'F', 12094, 3
    ),
    (
        473470, 'Baudoin', 'Ramsdale', 'bramsdale44@nbcnews.com', '2012/12/14', 'M', 56870, 2
    ),
    (
        741578, 'Maxie', 'Bulled', 'mbulled45@redcross.org', '2020/01/03', 'M', 48422, 5
    ),
    (
        712090, 'Georg', 'Sibbons', 'gsibbons46@blogger.com', '2012/03/21', 'M', 30415, 5
    ),
    (
        668586, 'Collette', 'Schwandner', 'cschwandner47@bloomberg.com', '2017/02/06', 'F', 46366, 4
    ),
    (
        297489, 'Travers', 'Baldung', 'tbaldung48@i2i.jp', '2011/12/04', 'M', 11880, 5
    ),
    (
        542379, 'Bertha', 'Brocking', 'bbrocking49@taobao.com', '2013/03/07', 'F', 58477, 4
    ),
    (
        241480, 'Boothe', 'Skule', NULL, '2021/12/23', 'M', 33355, 2
    ),
    (
        855941, 'Gwenny', 'Thurby', 'gthurby4b@bbc.co.uk', '2016/03/02', 'F', 15971, 2
    ),
    (
        786243, 'Ansley', 'ODonnell', 'aodonnell4c@loc.gov', '2014/01/20', 'F', 10466, 5
    ),
    (
        512102, 'Katrine', 'Rochell', 'krochell4d@wikispaces.com', '2013/03/06', 'F', 24967, 4
    ),
    (
        255742, 'Kelvin', 'Camelia', 'kcamelia4e@soundcloud.com', '2017/04/09', 'M', 40002, 5
    ),
    (
        39561, 'Purcell', 'Dillintone', 'pdillintone4f@ft.com', '2022/04/10', 'M', 13372, 2
    ),
    (
        136795, 'Danit', 'Pacht', 'dpacht4g@nymag.com', '2021/02/03', 'F', 21420, 3
    ),
    (
        557343, 'Belle', 'Kem', 'bkem4h@hexun.com', '2021/02/13', 'F', 14360, 2
    ),
    (
        480613, 'Orion', 'Cokly', 'ocokly4i@virginia.edu', '2012/03/21', 'M', 56785, 3
    ),
    (
        373569, 'Louella', 'Edlin', 'ledlin4j@purevolume.com', '2018/01/04', 'F', 56112, 1
    ),
    (
        744451, 'Sydney', 'Durward', 'sdurward4k@joomla.org', '2023/04/04', 'F', 41382, 4
    ),
    (
        770630, 'Caria', 'Gerg', NULL, '2013/04/30', 'F', 19040, 5
    ),
    (
        124076, 'Marcie', 'McKnockiter', 'mmcknockiter4m@independent.co.uk', '2019/01/14', 'F', 63476, 4
    );
...
INSERT INTO
    employees
VALUES (
        517970, 'Calley', 'Seamon', 'cseamon4n@jugem.jp', '2019/11/16', 'F', 19608, 5
    ),
    (
        879562, 'Fiann', 'Gubbins', 'fgubbins4o@dmoz.org', '2020/01/06', 'F', 62273, 3
    ),
    (
        424028, 'Tam', 'Pinchbeck', 'tpinchbeck4p@shop-pro.jp', '2018/08/26', 'M', 34330, 2
    ),
    (
        72729, 'Isabella', 'Izhak', 'iizhak4q@facebook.com', '2019/12/16', 'F', 11991, 3
    ),
    (
        978201, 'Granger', 'Hully', 'ghully4r@nytimes.com', '2021/01/04', 'M', 32387, 1
    ),
    (
        612781, 'Farrand', 'Tumpane', 'ftumpane4s@1und1.de', '2011/08/12', 'F', 11965, 4
    ),
    (
        154744, 'Noel', 'Sapir', NULL, '2014/10/22', 'F', 18370, 4
    ),
    (
        492531, 'Carmina', 'Grieswood', 'cgrieswood4u@diigo.com', '2022/04/24', 'F', 17007, 4
    ),
    (
        776952, 'Alick', 'Sambrook', 'asambrook4v@xrea.com', '2021/03/27', 'M', 59638, 1
    ),
    (
        89869, 'Julio', 'Clemerson', 'jclemerson4w@marketwatch.com', '2013/04/16', 'M', 38829, 4
    ),
    (
        905729, 'Mellisent', 'Allot', 'mallot4x@nytimes.com', '2012/10/22', 'F', 16126, 5
    ),
    (
        519735, 'Shari', 'Devonport', 'sdevonport4y@pcworld.com', '2014/06/27', 'F', 52879, 1
    ),
    (
        287914, 'Hamel', 'Vineall', NULL, '2011/08/08', 'M', 58278, 1
    ),
    (
        881778, 'Berny', 'Krzyzaniak', 'bkrzyzaniak50@netvibes.com', '2016/07/19', 'M', 59095, 2
    ),
    (
        668303, 'Lettie', 'Christauffour', 'lchristauffour51@yelp.com', '2011/05/20', 'F', 53830, 1
    ),
    (
        438933, 'Aubrey', 'Webburn', 'awebburn52@hostgator.com', '2012/12/02', 'F', 58367, 2
    ),
    (
        182181, 'Arlin', 'Vernay', NULL, '2022/10/29', 'M', 17068, 2
    ),
    (
        988100, 'Sibylla', 'Fordham', 'sfordham54@shinystat.com', '2021/07/13', 'F', 39946, 3
    ),
    (
        511470, 'Pate', 'Denness', 'pdenness55@constantcontact.com', '2010/11/07', 'M', 37567, 2
    ),
    (
        552104, 'Chickie', 'Attaway', 'cattaway56@bravesites.com', '2018/07/03', 'M', 34702, 5
    ),
    (
        43663, 'Doug', 'Sogg', 'dsogg57@house.gov', '2017/12/10', 'M', 12586, 4
    ),
    (
        923916, 'Butch', 'Bichard', 'bbichard58@plala.or.jp', '2023/02/10', 'M', 10875, 3
    ),
    (
        263003, 'Maybelle', 'Twiggins', 'mtwiggins59@nymag.com', '2014/06/01', 'F', 13216, 3
    ),
    (
        407248, 'Lionel', 'Po', 'lpo5a@ucoz.ru', '2022/12/03', 'M', 42997, 4
    ),
    (
        996837, 'Norene', 'Bonnick', NULL, '2017/08/17', 'F', 63340, 2
    ),
    (
        261401, 'Lizette', 'Dominetti', 'ldominetti5c@chron.com', '2016/01/19', 'F', 15416, 4
    ),
    (
        515402, 'Natalee', 'Rohfsen', NULL, '2022/10/31', 'F', 61694, 2
    ),
    (
        299527, 'Felice', 'Mooring', NULL, '2010/02/12', 'F', 19639, 5
    ),
    (
        146381, 'Stinky', 'Tyt', NULL, '2018/09/06', 'M', 33835, 2
    ),
    (
        323891, 'Ryon', 'Kislingbury', NULL, '2022/02/08', 'M', 33139, 3
    ),
    (
        130514, 'Raddie', 'Lishmund', 'rlishmund5h@hud.gov', '2020/10/18', 'M', 59407, 1
    ),
    (
        295409, 'Paco', 'Eick', 'peick5i@nasa.gov', '2020/04/17', 'M', 59607, 2
    ),
    (
        500970, 'Rebbecca', 'Karpychev', 'rkarpychev5j@com.com', '2021/04/27', 'F', 16014, 4
    ),
    (
        324955, 'Henryetta', 'Landrieu', 'hlandrieu5k@illinois.edu', '2013/02/04', 'F', 53993, 2
    ),
    (
        334640, 'Lurette', 'Luthwood', 'lluthwood5l@istockphoto.com', '2016/08/04', 'F', 40792, 1
    ),
    (
        811551, 'Cob', 'Lynes', 'clynes5m@nydailynews.com', '2019/12/05', 'M', 23383, 4
    ),
    (
        82410, 'Fitz', 'Pigrome', 'fpigrome5n@1688.com', '2021/09/16', 'M', 12977, 4
    );
